function delete_career_information(id) {
    $.alert({
        title: '<h3 class="text-dark text-center text-monospace mb-1 mt-2">Confirmation</h3>',
        content: '<div>Are you sure you want to delete this Career Information?</div>',
        buttons:{
            OK: function () {
                $.ajax({
                    url: "../../assets/utils/admin_portal/delete_career_information.php",
                    type: 'POST',
                    data: {
                        id: id,
                    },
...
$(document).ready(function() {
    $('#dataTable').DataTable({
        ...
        language: {
            emptyTable: function(){
                return "No Career Information uploaded yet";
            }
        },
        pagelength: 25,
        paging: true,
        ajax: {
            "url": "../../assets/utils/admin_portal/get_career_information.php",
            dataSrc: "career_information",
        },
        columns: [
            {data: "DT_RowId"},
            {data: "title"},
            ...
            {data: "DT_RowId",
                render: function(data) {
                    return('<a href="edit_career_information.php?id=' + data + '" class="btn btn-sm btn-primary mx-1 text-white">Edit</a>' +
                    '<button onclick="delete_career_information(\'' + data + '\')" class="btn btn-sm btn-danger mx-1">Delete</button>');
                }
            }
        ],
        columnDefs: [
