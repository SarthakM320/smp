...
function handleMouseMove(event) {
	mousePos = {x:event.clientX, y:(event.clientY - window.innerHeight/2)};
}
...
function handleTouchStart(event) {
	if (event.touches.length > 1) {
		...
		mousePos = {x:event.touches[0].pageX, y:(event.touches[0].pageY - window.innerHeight/2)};
		isBlowing = true;
	}
}
...
function createFloor(){
	floor = new THREE.Mesh(new THREE.PlaneBufferGeometry(1000,500), new THREE.MeshBasicMaterial({color: 0x385796}));
