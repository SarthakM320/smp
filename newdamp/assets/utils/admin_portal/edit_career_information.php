...
function edit_career_information() {
    try{
        $id = intval($_POST["id"]);
        ...
        $link = linktoDAMP();
        # Fetch previous details of career information
        ...
        if($prev_category1 !== $category1){
            ...
            $query4 = "SELECT COUNT(id) AS count FROM `career_information` WHERE `category1` = :category1 AND `branch` = :branch";
        ...
        $query6 = "UPDATE `career_information` SET `title` = :title, `content` = :content, `category1` = :category1, `category2` = :category2 WHERE id = :id";
