...
function init() {
    $.ajax({
        url: "../../assets/utils/admin_portal/edit_career_information_init.php",
        data: {
        ...
        success: function(result) {
            ...
            if(status === "S"){
                let career_information = result_decoded.career_information;
                let categories1 = result_decoded.categories1;
                categories2 = result_decoded.categories2;

                $('#title').val(career_information.title);
                $('#content').val(career_information.content);

                let category1_id = Number(career_information.category1);
                let category2_id = Number(career_information.category2);
    ...
    $('#edit').click(function(e) {
        ...
        if(is_valid){
            $.alert({
                ...
                content: '<div>Are you sure you want to edit this Career Information?</div>',
                buttons:{
                    OK: function() {
                        ...
                        $.ajax({
                            url: "../../assets/utils/admin_portal/edit_career_information.php",
                            type: "POST",
                            data: {
                                id: getUrlParameter("id"),
                                title: title,
