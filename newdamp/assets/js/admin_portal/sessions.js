function delete_session(id) {
    $.alert({
        ...
        content: '<div>Are you sure you want to delete this Session?</div>',
        buttons:{
            OK: function () {
                $.ajax({
                    url: "../../assets/utils/admin_portal/delete_session.php",
                    type: 'POST',
                    data: {
                        id: id,
                    },
...
$(document).ready(function() {
    $('#dataTable').DataTable({
        ...
        paging: true,
        ajax: {
            "url": "../../assets/utils/admin_portal/get_sessions.php",
            dataSrc: "sessions",
        },
        columns: [
            {data: "DT_RowId"},
            ...
            {data: "DT_RowId",
                render: function(data) {
                    return('<a href="edit_session.php?id=' + data + '" class="btn btn-sm btn-primary mx-1 text-white">Edit</a>' +
                    '<button onclick="delete_session(\'' + data + '\')" class="btn btn-sm btn-danger mx-1">Delete</button>');
                }
            }
        ],
        columnDefs: [
