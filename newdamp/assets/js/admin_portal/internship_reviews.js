function delete_internship_review(id) {
    $.alert({
        title: '<h3 class="text-dark text-center text-monospace mb-1 mt-2">Confirmation</h3>',
        content: '<div>Are you sure you want to delete this Internship Review?</div>',
        buttons:{
            OK: function () {
                $.ajax({
                    url: "../../assets/utils/admin_portal/delete_internship_review.php",
                    type: 'POST',
                    data: {
                        id: id,
                    },
...
$(document).ready(function() {
    $('#dataTable').DataTable({
        ...
        language: {
            emptyTable: function(){
                return "No Internship Reviews uploaded yet";
            }
        },
        pagelength: 25,
        paging: true,
        ajax: {
            "url": "../../assets/utils/admin_portal/get_internship_reviews.php",
            dataSrc: "internship_reviews",
        },
        columns: [
            {data: "DT_RowId"},
            {data: "title"},
            ...
            render: function(data) {
                return('<a href="edit_internship_review.php?id=' + data + '" class="btn btn-sm btn-primary mx-1 text-white">Edit</a>' +
                '<button onclick="delete_internship_review(\'' + data + '\')" class="btn btn-sm btn-danger mx-1">Delete</button>');
                }
            }
        ],
        columnDefs: [
